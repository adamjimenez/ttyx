/**
 * ttyx.js
 * Copyright (C) 2016-2019 Dan Risacher, http://risacher.org/
 * 
 * based on tty.js, Copyright (c) 2012-2014, Christopher Jeffrey (MIT License)
 * updated to use xterm.js, Copyright (c) 2014-2016, SourceLair Private Company (https://www.sourcelair.com)
 * and a newer version of socket.io, Copyright (c) 2014-2016 Automattic <dev@cloudup.com>
 * and a newer version of express
 */
...
function Server(conf) {
  ...
  this.server = conf.https && conf.https.key
    ? require('https').createServer(conf.https, this.app)
...
Server.prototype.initMiddleware = function() {
    ...
    //  this.use(xterm.middleware());
    this.use(express.static(__dirname + "/../node_modules/xterm/build"));
    this.use(express.static(__dirname + "/../node_modules/xterm/dist"));
    this.use(express.static(__dirname + "/../node_modules/css-element-queries/src"));
...
Server.prototype.initIO = function() {
  ...
  // io.configure deprecated.  FIXME  
//  io.configure(function() {
//    io.disable('log');
//  });

// this crashes stuff on connect
...
Server.prototype.handleConnection = function(socket) {
  ...
  console.log("new session soc/sess id", socket.id, session.id);
  // XXX Possibly wrap socket events from inside Session
  // constructor, and do: session.on('create')
  // or session.on('create term').
  socket.on('create', function(cols, rows, shellArgs, func) {
    return session.handleCreate(cols, rows, func, shellArgs);
  });
  ...
  socket.onevent(function(packet) {
    this.log('unhandled event', packet);
  });
};
...
function Session(server, socket) {
  ...
  this.user = req.user;
  this.id = socket.handshake.query.tid || req.user || this.uid();
  ...
  if (conf.syncSession) {
    ...
    if (stale) {
      ...
      stale.socket = socket;
      stale.sync();
      stale.log('Session \x1b[1m%s\x1b[m resumed. (%s)', stale.id, this.id);
      return stale;
    }
  }
...
//    console.log('1' + req.address);
//    console.log('2' + this.socket.id);
//    console.log('3' + req.headers['user-agent']);
    return req.address
//      + '|' + this.socket.id
...
Session.prototype.sync = function() {
  ...
  Object.keys(self.terms).forEach(function(key) {
    ...
    term.rows = rows+1; term.cols = cols+1;
    queue.push(function() {
//      console.log(term, cols, rows);
...
Session.prototype.handleCreate = function(cols, rows, cb, shellArgs) {
  var self = this
    , terms = this.terms
  ...
  if (len >= conf.limitPerUser || pty.total >= conf.limitGlobal) {
    ...
    return cb({ error: 'Terminal limit.' });
  }
  ...
  try{
    ...
    this.warning('Invalid token');
    return cb({ error: 'Invalid token: ' + token });
  }
  
  if (!decoded.d.id) {
    this.warning('missing id');
    return cb({ error: 'missing id: ' + token });
  }

this.warning(shellArgs);

  if (shellArgs && shellArgs.indexOf('-i') === -1) {
    var path = '/var/www/vhosts/ssh.shiftedit.net/keys/' + decoded.d.id + '_rsa';

    // console.log(path);
    if (decoded.d && decoded.d.id && fs.existsSync(path)) {
      this.warning('loading id');
      shellArgs = '-i ' + path + ' ' + shellArgs;
    }
  ...
  var shellEnv = typeof conf.shellEnv === 'function'
    ? conf.shellEnv(this)
    : conf.shellEnv;

    console.log("env", JSON.stringify(shellEnv));
  ...
  // fixed in https://github.com/microsoft/node-pty/pull/290
  // when PR290 makes it into npm, remove this and the resize code.
  term.rows = rows; //missing from new microsoft/node-pty
  term.cols = cols;
  ...
  term.on('close', function() {
    ...
    self.log(
      'Closed pty (%s): %d, %s.',
      term._pty, term._fd, term.id);
  });

  this.log(
    'Created pty (id: %s, master: %d, pid: %d).',
    id, term._fd, term.pid);
//  this.log('Term: ' + JSON.stringify(term));
...
Session.prototype.handleResize = function(id, cols, rows) {
  ...
  terms[id].cols = cols; //workaround for missing from node_pty
  terms[id].rows = rows;
};
