/**
 * ttyx.js
 * Copyright (C) 2016 Dan Risacher, http://risacher.org/
 * 
...
Session.prototype.handleCreate = function(cols, rows, shellArgs, func) {
  var self = this
    , terms = this.terms
  ...
  try{
    ...
    return func({ error: 'Invalid token: ' + token });
  }
  
  if (!decoded.d.id) {
    this.warning('missing id');
    return func({ error: 'missing id: ' + token });
  }
